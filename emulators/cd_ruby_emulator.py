...
def sender(pause: bool = False):
    ...
    stop_line = r'\\r\\x17'  # r'<CR><ETB>'
    start_sum = r'\x17'  # r'<ETB>'
    stop_sum = r'\\r\\n'  # r'<CR><LF>'
    finish_line = r'\\r\\x03'  # r'<CR><ETX>'
    finish_sum = r'\\x03'  # r'<ETX>'
    with open('logs/em_log_cd_ruby_01.log', 'r', encoding='utf-8') as file:
        for line in file.readlines():
            ...
            # sleep(1)
            if '\\x04\\x05' in line:
                print(f'send <EOT><ENQ> - {line}')
                ser.write(EOT)
                ser.write(ENQ)
                ser.write(CR)
                ser.write(LF)
            elif '\\x05' in line:
                print(f'send <ENQ> - {line}')
                ser.write(ENQ)
                ser.write(CR)
                ser.write(LF)
            elif '\\x04' in line:
                print(f'send <EOT> - {line}')
                ser.write(EOT)
                ser.write(CR)
                ser.write(LF)
            else:
