...
# from driver_rs232_port.drivers.driver_rs232_port.models import Analyzes, add_analyze, delete_analyze, edit_analyze
from driver_rs232_port.models import Analyzes, add_analyze, delete_analyze, edit_analyze
...
from driver_rs232_port.drivers.bs_240.handler_soap import create_soap
from driver_rs232_port.drivers.bs_240.handler_worklist import main as worklist_commands_for_driver
from driver_rs232_port.drivers.bs_240.getter_worklist import get_worklist_from_lis
        ...
        if find_enq >= 0 and find_eot > 0 and (find_enq < find_eot):
            ...
            if results:
                for n, result in enumerate(results):
                    if result.get('query_definition'):
                        ...
                        qck_message_list, dsr_message_list = worklist_commands_for_driver(result, lis_data, message_id)
                        logger.info(f"Worklist QCK data: {qck_message_list}")
                        logger.info(f"Worklist DSR data: {dsr_message_list}")
                        result['qck_message_list'] = qck_message_list
                        result['dsr_message_list'] = dsr_message_list
                        # worklist_test = test_worklist_commands_for_driver(result, lis_data)
                        # logger.info(f"Worklist test data for analyzer: {worklist_test}")
                        # result['worklist_test'] = worklist_test

                        send_data = ''.join(result['qck_message_list'])
                        logger.info(f"QCK message to Driver: {send_data}")
                        # отправка
                        ...
                        logger.info(f"Status of QCK message: {status}")

                        send_data = ''.join(result['dsr_message_list'])
                        logger.info(f"DSR message to Driver: {send_data}")
                        # отправка
                        status = send_to_driver(device_number=analyzer_number,
                                                device_name=analyzer_name,
                                                data=send_data.encode(),
                        ...
                        logger.info(f"Status of DSR message: {status}")

                        if isinstance(status, int) and status == 200:
                            semaphore.acquire()
                            delete_analyze(analyze=current_analyze)
                            semaphore.release()
