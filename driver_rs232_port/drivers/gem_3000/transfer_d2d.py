...
from main.drivers.driver_proxy.drivers.gem_3000.handler_soap import create_soap
...
# elif analyzer_name == 'gem3000':
#     if results:
#         # шаблон для формирования (текстового) отчета о результатах анализа последней полученной пробы
#         set_probes = f"set_probes_{analyzer_name}(data={results}, raw_line={raw_line}, device_id='{analyzer_number}')"
#         logger.info(f'SETTER PROBES: {set_probes}')
#         logger.info(f"def index() result: {results}")
#         # формирование (текстового) отчета о результатах анализа последней полученной пробы
#         results_text = eval(set_probes)
#         # если отчет сформирован
#         if results_text:
#             logger.info(f"ADD Data into driver's DB: "
#                         f"Probe: {results}, \n"
#                         f"Analyzer (name): {analyzer_name}, "
#                         f"Analyzer (number): {analyzer_number}")
#             # добавление результатов анализов в буферную БД
#             add_analyze(analyzer_name, analyzer_number, results_text)
#             # посылка данных в LIS
#             send_result = send_soap(semaphore)
#             return send_result
#         # если отчета нет
#         else:
#             logger.info(f"RAW LINE: {raw_line}")
#             logger.info(f"Data results not added!")
#             send_result = {'status': "Data results not added!"}
#             return send_result
